function [Wopt,lambdaopt,RMSEval,RMSEest] = multiframe_lasso_cvi(T,X,lambdavec,K)
...
for kframe = 1:Nframes % First loop over frames 
    for kfold = 1:K % Then loop over the folds
        ...
        for klam = 1:Nlam  % Finally loop over the lambda grid
            ...
            what = lasso_ccdi(t(estind), X(estind,:), lambdavec(klam), wold);% Calculate LASSO estimate at current frame, fold, and lambda
            SEval(kfold,klam) = SEval(kfold,klam) + norm(t(valind)-X(valind,:)*what)^2; % Add validation error at current frame, fold and lambda to the validation error for this fold and lambda, summing tthe error over the frames
            SEest(kfold,klam) = SEest(kfold,klam) + norm(t(estind)-X(estind,:)*what)^2; % Add estimation error at current frame, fold and lambda to the estimation error for this fold and lambda, summing tthe error over the frames
...
for kframe = 1:Nframes
    ...
    Wopt(:,kframe) = lasso_ccdi(t,X,lambdaopt, wold);
    framelocation = framelocation + framehop;
end
